...
<div class="bjui-pageContent">
	<form id="roleAddForm" action="<%=basePath%>/system/role/save.do"
		...
		<div
			...
			<div class="form-group">
				<label class="control-label x90">角色代码：</label> <input type="text"
					...
					data-rule="required length[~32] "
					size="20" />
				<%-- <label class="control-label x90">角色代码：</label> <input type="text"
					name="roleCode" id="roleCode" value=""
					data-rule="required length[~32] remote[<%=basePath%>/system/role/validateRoleCode.do]"
					size="20" /> --%>
			</div>
			<div class="form-group">
					...
					<ul id="ruleSelectTree" class="ztree" data-toggle="ztree"
						...
						<c:forEach items="${treeList}" var="t" varStatus="s">
							<li data-id="${t.id}" data-pid="${t.parentId}"
								data-faicon="${t.imgPath}">${t.name}</li>
						</c:forEach>
					</ul>
				</div>
			</fieldset>
...
<div class="bjui-pageFooter">
	<ul>
		...
		<li><button type="button" class="btn-default"
				onclick="submitAddForm();">保存</button></li>
	</ul>
</div>
